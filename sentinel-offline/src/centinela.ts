...
function launcherRunning(root:string){const probe=spawnSync(join(root,"scripts","medos-launcher-control"),["status"],{cwd:root,encoding:"utf8",shell:false,timeout:3000});return probe.status===0}
...
function status(root:string):Result{
 ...
 return result("status",{repository:basename(root),branch,head,workingTree:porcelain?"modified":"clean",origin:{upstream,sync},node:process.version,pnpm:pnpmVersion,launcher:{installed:existsSync(join(root,"sentinel-offline","launcher","package.json")),running:launcherRunning(root)},n8n:{installed:existsSync(join(root,"sentinel-offline","orchestrator","package.json")),running:false,bind:"127.0.0.1:5678",workflows:"inactive"},health:"local",bind:"127.0.0.1",vault:{available:false,selected:false},persistence:"BLOCKED_WITHOUT_VAULT",connections:"BLOCKED",backup:"requires-encrypted-vault",lastRestoreTest:null,lastCheckpoint:cp?"manifest-present":null,nextTask:"Validación humana de Termux y bóveda",ci:"not-queried",codeql:"not-queried"},"WARN",["Bóveda no seleccionada; operaciones sensibles bloqueadas"],"Seleccione una bóveda mediante el Launcher cuando esté disponible");
}
